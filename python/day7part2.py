#!/usr/bin/python3
# DAY 7 PART 2 - BRIDGE REPAIR CONVOLUTION
#
# Given a sequence consisting of :
#  RESULT: [operand] [operand] ...
#
# ...Determine the +,*,| operators that can go between the operands to produce the RESULT
...
# The "|" operator just concatenates two operands: '6 | 7' = 67
...
def evaluate(operands,operators) :
    ...
    for i in range(1,len(operands)) :
        if operators[i-1] == '+' :
            ...
            sum = int(str(sum) + str(operands[i]))
        else :
            sum = sum * operands[i]
    return sum
...
if __name__ == "__main__" :
    ...
    with open(df, 'r') as n:
        for l in n:
            ...
            tcount += 1
            if tcount%10 == 0 :
                print(f"{tcount}  complete {tcount/850:.1%}")
    print(f"PART 2 ANSWER: {sumoftrueresults}")
